...
class PraDiSyAfterAction extends PluginBase
{
    ...
    //        }
        ...
        . 'Bitte notieren sie sich Ihre ID: <strong style="color:red;">' . $response['participant_id']  . '</strong><p> und ihren Zugangscode: <strong style="color:red;">' . $tokenObject . '</strong><p>'
        ...
        Yii::log('JUTTA ' . CVarDumper::dumpAsString($oFormattingOptions), CLogger::LEVEL_INFO, 'application');
        ...
        Yii::log("FUCKINGTASTIC FILE", CLogger::LEVEL_INFO, 'application');         
    ...
    public function updateCsvToken($csvFile, $tokenValue)
    {
        ...
        if (($handle = fopen($csvFile, 'r')) !== false) {
            ...
            if ($tokenIndex === false) {
                Yii::log("Spalte 'token' wurde nicht gefunden in " . $csvFile, CLogger::LEVEL_ERROR, 'application');
                return false;
            }
            ...
            if (($handle = fopen($csvFile, 'w')) !== false) {
                ...
                fputcsv($handle, $data, ';');
                fclose($handle);
                Yii::log("CSV-Datei " . $csvFile . " wurde erfolgreich aktualisiert.", CLogger::LEVEL_INFO, 'application');
                return true;
            } else {
                Yii::log("Fehler beim Öffnen der Datei zum Schreiben: " . $csvFile, CLogger::LEVEL_ERROR, 'application');
                return false;
            }
        } else {
            Yii::log("Fehler beim Öffnen der Datei zum Lesen: " . $csvFile, CLogger::LEVEL_ERROR, 'application');
            return false;
        }
    }
    ...
    private function sendSurveyCompletionMail($args)
    {
        ...
        Yii::log('DEBUGGING LASAGNE SMTP-Konfiguration:', CLogger::LEVEL_INFO, 'application');
        Yii::log('DEBUGGING LASAGNE Host: ' . $smtpHost, CLogger::LEVEL_INFO, 'application');
        Yii::log('DEBUGGING LASAGNE Port: ' . $smtpPort, CLogger::LEVEL_INFO, 'application');
        Yii::log('DEBUGGING LASAGNE Username: ' . $smtpUsername, CLogger::LEVEL_INFO, 'application');
        Yii::log('DEBUGGING LASAGNE smtpPassword: ' . $smtpPassword, CLogger::LEVEL_INFO, 'application');        
        Yii::log('DEBUGGING LASAGNE Encryption: ' . $smtpEncryption, CLogger::LEVEL_INFO, 'application');
        ...
        try {
            ...
            Yii::log('PASTAFARI SMTP: Initialisiere Mailer...', CLogger::LEVEL_INFO, 'application');
            Yii::log('PASTAFARI SMTP: Setze Mailer auf isSMTP()...', CLogger::LEVEL_INFO, 'application');
            ...
            Yii::log('SMTP: $mail->Body ...' . $mail->Body, CLogger::LEVEL_INFO, 'application');

            $mail->addAttachment($datafile); 

            Yii::log('SMTP: Bereite E-Mail zum Senden vor...', CLogger::LEVEL_INFO, 'application');
