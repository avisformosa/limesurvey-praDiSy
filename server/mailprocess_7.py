        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                ...
                if parsed_data:
                    ...
                    filepath = save_attachment_with_survey_id(msg, INPUT_DIR, diag_id)
                    if filepath:
                        log_debug(f"Saved attachment: {filepath}")

                        # Mail-ID in die CSV schreiben
                        try:
                            data = pd.read_csv(filepath, delimiter=";", encoding="utf-8")
                            data['email_id'] = email_id  # Mail-ID als neue Spalte hinzufügen
                            data.to_csv(filepath, sep=";", index=False)  # Überschreibt die bestehende CSV
                        except Exception as e:
                            log_debug(f"Fehler beim Bearbeiten der CSV {filepath}: {e}")
                            continue

                        # Timestamp und andere Informationen aus der CSV extrahieren
                        try:
                            timestamp = data['submitdate'].iloc[0]  # Zeitstempel aus der CSV
                        except KeyError:
                            log_debug(f"Spalte 'submitdate' fehlt in der CSV {filepath}.")
                            continue

                        # process_file mit allen relevanten Informationen aufrufen
                        status = process_file(filepath, diag_id, participant_id, timestamp)
                        log_debug(f"Status der Verarbeitung für Datei {filepath}: {status}")
