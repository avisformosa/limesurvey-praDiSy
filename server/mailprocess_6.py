...
def process_tobeprocessed():
    """Überprüft das Verzeichnis `tobeprocessed` und verarbeitet Dateien."""
    for file_name in os.listdir(INPUT_DIR):
        file_path = os.path.join(INPUT_DIR, file_name)

        # Überspringe Systemdateien oder versteckte Dateien
        if file_name.startswith('.') or not file_name.endswith('.csv'):
            log_debug(f"Ignoriere Datei: {file_name}")
            continue

        if os.path.isfile(file_path):
            log_debug(f"Verarbeite Datei aus tobeprocessed: {file_name}")
            
            # Survey ID aus der CSV-Datei extrahieren
            survey_id = extract_survey_id_from_csv(file_path)
            
            if survey_id:
                # Datei verarbeiten
                ## status = process_file(file_path, survey_id)
                # status = process_file(file_path, survey_id, participant_id, timestamp)
                # DIAG769569PARTICIPANT08bd93f6-0968-4260-8281-2a44f72a55fcDATE2024-12-31TIME09:25:10
                log_debug(f"Verarbeitung abgeschlossen: {file_name} mit Status {status}")
            else:
                log_debug(f"Keine survey_id in der Datei {file_name} gefunden. Überspringe.")
...
def check_mail():
    """Prüft neue E-Mails, speichert CSV-Anhänge und aktualisiert den Status."""
    log_debug("check_mail() is running...")
    mail = imaplib.IMAP4_SSL(IMAP_SERVER)
    mail.login(USERNAME, PASSWORD)
    mail.select("inbox")

    status, messages = mail.search(None, "UNSEEN")
    log_debug("Lines: " + str(len(messages[0].split())))
    ...
    for mail_id in mail_ids:
        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                ...
                parsed_data = subject_pattern.match(subject)
                if parsed_data:
                    metadata = parsed_data.groupdict()
                    diag_id = metadata['diag_id']
                    insert_email_metadata(email_id, diag_id, metadata['participant_id'], metadata['date'], metadata['time'])
                    # filepath = save_attachment(msg, INPUT_DIR)
                    # filepath = save_attachment_with_survey_id(msg, INPUT_DIR, metadata['diag_id'])
                    filepath = save_attachment_with_survey_id(msg, INPUT_DIR, metadata['diag_id'])



                    if filepath:
                        log_debug("Processing file: " + filepath + " for email ID: " + email_id)
                        mail.store(mail_id, '+FLAGS', '\\Seen')
                        # status = process_file(filepath, diag_id)
                        #log_debug("file_path: " + file_path + " with survey_id: " + survey_id + " and participant_id: " + participant_id + " and timestamp: " + timestamp)
                        # status = process_file(file_path, survey_id, participant_id, timestamp)
                        # update_email_status(email_id, status)
                else:
                    log_debug("Invalid email subject format.")
    mail.logout()
