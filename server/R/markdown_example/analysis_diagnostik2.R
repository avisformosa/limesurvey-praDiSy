...
cat(file.path(get_script_dir(), "server/R/markdown_example/diagnostik.qmd"))

# Basiskommando, Quarto soll "report.qmd" als PDF rendern:
quarto_args <- c(
  "render",        # Befehl: quarto render
  file.path(get_script_dir(), "server/R/markdown_example/diagnostik.qmd"),    # unsere Quarto-Datei
...
system2("quarto", c(
  "render", "server/R/markdown_example/diagnostik.qmd", "--to", "pdf",
  "--execute-param", "patient_name='Herr Müller'",
  "--execute-param", "score_value=99"
))
